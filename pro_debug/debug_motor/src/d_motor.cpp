...
//	std::lock_guard<mutex> lock(m_mutex);
	...
	if(frame[0]==0x72)
	{
	  	motor_rpm[0] = (int32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
	  	motor_Nm[0] =(int32_t)frame[5]|(frame[6]<<8)|(frame[7]<<16)|(frame[8]<<24);
	  	ROS_INFO("motor1_rpm = %d,motor1_Nm = %d",motor_rpm[0],motor_Nm[0]);
	}
	else if(frame[0]==0x73)
	{
		motor_rpm[1] =(int32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
	  	motor_Nm[1] =(int32_t)frame[5]|(frame[6]<<8)|(frame[7]<<16)|(frame[8]<<24);
	}
	else if(frame[0]==0x74)
	{
	 	motor_rpm[2] = (int32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
	  	motor_Nm[2] = (int32_t)frame[5]|(frame[6]<<8)|(frame[7]<<16)|(frame[8]<<24); 		
	}
	else if(frame[0]==0x75)
	{
	 	motor_rpm[3] = (int32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
	  	motor_Nm[3] = (int32_t)frame[5]|(frame[6]<<8)|(frame[7]<<16)|(frame[8]<<24); 			
	}
	else if(frame[0]==0x76)
	{
	 	motor_rpm[4] = (int32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
	  	motor_Nm[4] = (int32_t)frame[5]|(frame[6]<<8)|(frame[7]<<16)|(frame[8]<<24);		
	}
	else if(frame[0]==0x77)
	{
	 	motor_rpm[5] = (int32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
	  	motor_Nm[5] =(int32_t)frame[5]|(frame[6]<<8)|(frame[7]<<16)|(frame[8]<<24);		
	}
	else if(frame[0]==0x52)		//
	{
		motor_break_code[0]= (uint32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
		motor_tempoc[0]=frame[5];	//frame[6]  mdriver_tempoc;
		mdriver_tempoc[0]=frame[6];
		motor_odom_er[0]=(uint16_t)frame[7]|(frame[8]<<8);	
//		printf("\nframe[5] = %.2x frame[6] = %.2x motor_tempoc= %.2x \n",frame[5],frame[6],motor_tempoc[0]);
	...
	{
		motor_break_code[1]= (int32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
		motor_tempoc[1]=frame[5];	//frame[6]  mdriver_tempoc;
		mdriver_tempoc[1]=frame[6];
		motor_odom_er[1]=(uint16_t)frame[7]|(frame[8]<<8);	
	}
	else if(frame[0]==0x54)
	{
		motor_break_code[2]= (int32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
		motor_tempoc[2]=frame[5];	//frame[6]  mdriver_tempoc;
		mdriver_tempoc[2]=frame[6];
		motor_odom_er[2]=(uint16_t)frame[7]|(frame[8]<<8);	
	}
	else if(frame[0]==0x55)
	{
		motor_break_code[3]= (int32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
		motor_tempoc[3]=frame[5];	//frame[6]  mdriver_tempoc;
		mdriver_tempoc[3]=frame[6];
		motor_odom_er[3]=(uint16_t)frame[7]|(frame[8]<<8);	
	}
	else if(frame[0]==0x56)
	{
		motor_break_code[4]= (int32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
		motor_tempoc[4]=frame[5];	//frame[6]  mdriver_tempoc;
		mdriver_tempoc[4]=frame[6];
		motor_odom_er[4]=(uint16_t)frame[7]|(frame[8]<<8);	
	}
	else if(frame[0]==0x57)
	{
		motor_break_code[5]= (int32_t)frame[1]|(frame[2]<<8)|(frame[3]<<16)|(frame[4]<<24);
		motor_tempoc[5]=frame[5];	//frame[6]  mdriver_tempoc;
		mdriver_tempoc[5]=frame[6];
		motor_odom_er[5]=(uint16_t)frame[7]|(frame[8]<<8);	
	}
	else;
	m_mutex.lock();
	for(int idx=0;idx<6;idx++)
	{
		...
		motor_Nm_[idx]=motor_Nm[idx]/65536*700;		//65536
...
//	static int64_t analyse_CAN_data_counter=0;
//  	while(nh_.ok())
//  	{
...
//  		ROS_INFO("\033[32manalyse_data with size: \033[0m%d",static_cast<int>(received_data_.data.size()));
	  	...
	  	for(int idx=0;idx<r_length;idx++)
	  	{
	  		...
	  		else if(r_data[idx]==0x33)
	  		{
	  			...
	  			if(idx>=r_length-8)return 4;
	  		}
	  		else if(r_data[idx]==0x77)
	  		{
//	  			if(oper_object_==9)	//batch read
//	  			{
//	  				memcpy(*(params_+params_index_),r_data,r_length);	//存储batch read 后的数据
...
//					ROS_INFO("\033[32m%dth response in terms of batch write\033[0m",params_index_);
//	  			}
	  			ROS_INFO("got command with respect to \033[32mshake hand command\033[0m value operation");
	  			if(idx>=r_length-8)return 4;
	  		}
	  		//08 00 00 00 72 00 00 00 00 00 00 00 00 
...
////	  			analyse_CAN_data_counter++;
////	  			if(((r_data[idx]&0xf0)==0x52)||((r_data[idx]&0xf0)==0x53)||((r_data[idx]&0xf0)==0x54)||((r_data[idx]&0xf0)==0x55)||((r_data[idx]&0xf0)==0x56)||((r_data[idx]&0xf0)==0x57))
//				if(idx>=r_length-8)
//				{
//					ROS_INFO("analyse CAN data from motor ko------- idx = %d ",idx);
//				  	return 5;
//				}
//			}
	  		else 
...
//					send_nbyte_with_response(0x33,page_idx,lword_idx,0,4);
		...
		outfile.open("/home/zt/catkin_work/devel/lib/data.txt");		//
		for(int idx=0;idx<1024;idx++)
			for(int sub_idx=0;sub_idx<8;sub_idx++)
			{
				outfile<<params_[idx][sub_idx]<<" ";
//				ROS_INFO("export data params_[%d][%d] = %d ",idx,sub_idx,params_[idx][sub_idx]);	
			}
		ROS_INFO("export data ko>>>>>>>>>>>>>>>>>>>>>");	
		outfile.close();
	}
...
void DebugMotor::control_motor()
{
	...
	setTorqueNm.val = set_torque_Nm_*65536/700;
	m_mutex.unlock();
	s_data[s_length++]=0x00;
...
//		s_data[s_length++]=(unsigned char)setSpeedNm&0xff;
	...
	else if(control_mode_==6)	//constant accelerate for speed
	{
		...
		static int32_t temp_speed_rpm=0;
		static uint8_t temp_flag=0;
		if(temp_flag==0)
		{	
			ROS_INFO("acceleration>>>>>>>>>>>>>>>>>>>>>>>>.");
			if(temp_speed_rpm==700)temp_flag=1;
			temp_speed_rpm+=1;
			ROS_INFO("\033[32mtemp_speed_rpm = %d \033[0m",temp_speed_rpm);
		...
		else if(temp_flag==1)
		{
			...
			if(temp_speed_rpm==-700)temp_flag=0;
			temp_speed_rpm-=1;
			ROS_INFO("\033[32mtemp_speed_rpm = %d \033[0m",temp_speed_rpm);
			set_speed_rpm_=temp_speed_rpm;
			setSpeedRpm.val=temp_speed_rpm*65536;
...
//		control_mode_=0;	
	...
	{
		static int32_t temp_torque_Nm=0;
		do
		{	
			temp_torque_Nm+=2;
			ROS_INFO("\033[32mtemp_torque_Nm = %d \033[0m",temp_torque_Nm);
...
//			control_motor();	//USB-CAN
			if(counter%50==0)
			{
				control_motor();	//USB-CAN test constant acceleration
//				syschronic_frame();
//				ROS_INFO("sent \033[32m%ldth\033[0m CAN frame.",send_CAN_counter_++);	
			}
		}
